document.querySelector("#search").addEventListener("submit", async (event) => {
  ...
  const key = "c2304ab01d249c41d718e156d8661a54";
  ...
  if (json.cod === 200) {
    ...
    document.querySelector("#weather").classList.remove("show");
    showAlert("Não foi possível localizar... <img src='Localizacao.png'>");
  }
});
...
function showInfo(json) {
  ...
  document.querySelector("#temp_value").innerHTML =
    json.temp.toFixed(1).toString().replace(".", ",") + "° C";
  document.querySelector("#temp_description").innerHTML = json.description;
  document.querySelector("#temp_img").src =
    `http://openweathermap.org/img/wn/${json.icon}@2x.png`;
  document.querySelector("#temp_max").innerHTML =
    json.tempMax.toFixed(1).toString().replace(".", ",") + "° C";
  document.querySelector("#temp_min").innerHTML =
    json.tempMin.toFixed(1).toString().replace(".", ",") + "° C";
